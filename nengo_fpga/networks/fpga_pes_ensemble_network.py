...
class FpgaPesEnsembleNetwork(nengo.Network):
    ...
    def connect_thread_func(self):
        ...
        local_data_filepath = os.path.join(self.arg_data_path,
                                           self.arg_data_file)
...
def build_FpgaPesEnsembleNetwork(model, network):
    ...
    if network.using_fpga_sim and network.config_found and network.fpga_found:
        ...
        # ens_args['eval_points'] = \
        #     param_model.params[network.ensemble].eval_points
        # ens_args['radius'] = network.ensemble.radius
        ...
        # conn_args['transform'] = \
        #     param_model.params[network.connection].transform
        # conn_args['solver'] = network.connection.solver
        # conn_args['synapse'] = network.connection.synapse
        ...
        data_filepath = os.path.join(network.arg_data_path, npz_filename)
        np.savez_compressed(data_filepath, sim_args=sim_args,
