...
class Simulator(nengo.simulator.Simulator):
    def __init__(self, network, **kwargs):
        ...
        for net in network.networks:
            if isinstance(net, FpgaPesEnsembleNetwork) and \
               net not in self.fpga_networks_list:
                ...
                for target in probe_target_list:
                    if isinstance(target, nengo.Ensemble) and \
                       target is net.ensemble:
                        raise nengo.exceptions.BuildError(
                            'FPGA PES Ensembles are currently non-probable.')
                    elif (isinstance(target, nengo.ensemble.Neurons) and
                          target.ensemble is net.ensemble):
                        raise nengo.exceptions.BuildError(
                            'FPGA PES Neurons are currently non-probable.')
                    elif (isinstance(target, nengo.Connection) and
                          target is net.connection):
                        raise nengo.exceptions.BuildError(
                            'FPGA PES Connections are currently ' +
                            'non-probable.')
